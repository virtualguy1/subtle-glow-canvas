  ...
  return <section id="tech-stack" className="section bg-secondary/30">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8">
          {Object.entries(skillsByCategory).map(([category, categorySkills], idx) => <div key={category} className="reveal-item opacity-0 translate-y-4 transition-all duration-700" style={{
          transitionDelay: `${idx * 100}ms`
        }}>
              <h3 className="text-xl font-semibold mb-4 text-accent">{category}</h3>
              <div className="flex flex-wrap gap-3">
                {categorySkills.map((skill, skillIdx) => <div key={skillIdx} className="group bg-card hover:bg-card/80 border border-border hover:border-accent/30 rounded-lg p-3 transition-all duration-300 shadow-sm hover:shadow flex items-center gap-3">
                    <span className="font-medium">{skill.name}</span>
                    <Badge variant="outline" className={`${getLevelColorClass(skill.level)} transition-colors`}>
        ...
        <div className="mt-8 reveal-item opacity-0 translate-y-4 transition-all duration-700">
          ...
          <div className="space-y-6">
            {Object.entries(techByCategory).map(([category, techs], idx) => <div key={category} className="transition-all duration-500" style={{
            transitionDelay: `${idx * 100}ms`
          }}>
                <h4 className="text-lg font-medium mb-3 text-muted-foreground">{category}</h4>
                <div className="flex flex-wrap gap-2">
                  {techs.map((tech, techIdx) => <Badge key={techIdx} variant="secondary" className="px-3 py-1.5 text-sm hover:bg-accent/10 hover:border-accent/30 transition-colors cursor-default">
                      {tech.name}
                    </Badge>)}
                </div>
              </div>)}
